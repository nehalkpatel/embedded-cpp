/* Recovered from this repository's history (commit fa430af) and modified.
   Original: STMicroelectronics startup_stm32f767xx.s, BSD-3-Clause.

   Local changes:
     - dropped `.fpu softvfp`, which contradicted the command-line FPU flags
     - `bl main` is followed by an infinite loop, not `bx lr`

   Kept rather than rewritten because the 110-entry interrupt vector table
   below must match the reference manual exactly, and transcribing it by hand
   buys nothing but the chance of a typo. */
/**
  ******************************************************************************
  ...
  */
  ...
  .syntax unified
  .cpu cortex-m7
  /* No .fpu directive: cmake/toolchain/armgcc-cm7.cmake puts
     -mfpu=fpv5-sp-d16 -mfloat-abi=hard on the command line, and a
     directive here would silently override it. */
  .thumb
...
/* Call the application's entry point.*/
  bl  main
/* main() does not return on bare metal. If it ever does, hang here rather
   than branching to whatever lr happened to hold. */
  b  .
