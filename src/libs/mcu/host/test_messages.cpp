...
TEST(EmulatorMessageJsonEncoderTest, EncodePinEmulatorRequest) {
  const PinEmulatorRequest request{MessageType::kRequest, ObjectType::kPin,
                                   "PA0", OperationType::kSet, PinState::kHigh};
...
TEST(EmulatorMessageJsonEncoderTest, DecodePinEmulatorRequest) {
  const std::string json{
      R"({"name":"PA0","object":"Pin","operation":"Set","state":"High","type":"Request"})"};
  const PinEmulatorRequest expected_request{
      MessageType::kRequest, ObjectType::kPin, "PA0", OperationType::kSet,
      PinState::kHigh};
